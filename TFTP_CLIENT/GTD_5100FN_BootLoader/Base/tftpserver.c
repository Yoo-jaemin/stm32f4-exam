...
static err_t IAP_tftp_send_ack_packet(struct udp_pcb *upcb, const ip_addr_t *to, int to_port, int block)
{
  ...
  //printf("IAP_tftp_send_ack_packet FUNC...\r\n\r\n");
...
{
  printf(" <3> IAP_wrq_recv_callback FUNC.......\r\n");
...
#endif
    printf("IAP_wrq_recv_callback FUNC Invalid data length...!!\r\n\r\n");
    return;
  }
...
static int IAP_tftp_process_write(struct udp_pcb *upcb, const ip_addr_t *to, int to_port)
{
  printf(" <2>  IAP_tftp_process_write FUNC!!\r\n\r\n");
...
static void IAP_tftp_recv_callback(void *arg, struct udp_pcb *upcb, struct pbuf *pkt_buf,
                        const ip_addr_t *addr, u16_t port)
{
  printf(" <1>  IAP_tftp_recv_callback FUNC!!\r\n\r\n");
...
void IAP_tftpd_init(void)
{
  ...
  if (!UDPpcb)
  {
    ...
    LCD_ErrLog("Can not create pcb \n");
#endif
    printf("Can not create pcb[new]\r\n\r\n");
    return;
  }
    printf("Can create pcb[new]\r\n\r\n");
  ...
  else
  {
    ...
    LCD_ErrLog("Can not create pcb \n");
#endif
    printf("Can not create pcb[bind]\r\n\r\n");
  }
}
