...
uint8_t GDI_Init( void )
{ 
	uint8_t		ret = GDI_ERROR;
	
	/* Default value for draw propriety */
		
	DrawProp.BackColor = BLACK;
	DrawProp.TextColor = WHITE;
	DrawProp.pFont     = &Font07x12;
	
	LcdDrive.Io_Init();
		
	if( LcdDrive.ReadID() == ILI9225G_ID ){
		lcd_drv = &LcdDrive;
...
void GDI_StatusArea_Clear(void)
{ 
  uint32_t counter = 0;
  uint32_t color_backup = DrawProp.TextColor; 
  DrawProp.TextColor = BLACK;
  
  for(counter = 126; counter < 161; counter++)
  {
    GDI_DrawHLine(28, counter, 152);
  }
  DrawProp.TextColor = color_backup; 
  GDI_SetTextColor(DrawProp.TextColor);
}

void GDI_ClearLine(uint16_t Ypos)
{
	//uint32_t counter = 0;
  	uint32_t color_backup = DrawProp.TextColor; 
 	 DrawProp.TextColor = BLACK;
 	...
 	//for(counter = 0; counter < GDI_GetYSize(); counter++)
  	//{
    	GDI_DrawHLine(Ypos, 1, GDI_GetXSize());
  	//}
  	DrawProp.TextColor = color_backup; 
 	GDI_SetTextColor(DrawProp.TextColor);
}
...
void GDI_String( uint16_t Xpos, uint16_t Ypos, GDI_LineModeType Mode, char *Text )
{
	...
	/* Get the text size */
	while( *ptr++ ){
		if( (*ptr == '.') && (DrawProp.pFont->pTable == Font29x48.pTable) ){
	...
	if( DrawProp.pFont->pTable == Font29x48.pTable ){
	...
	{                       
		if( (IsMesuringFontDotMode == true) && (*Text == '.') )	{ refcolumn -= 3;					  }
